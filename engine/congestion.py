...
def visualize_rudy_map(
    model: "BoardModel",
    grid_resolution: float = 2.0,
    path: str | None = None,
):
    """Visualize the RUDY congestion map as a heatmap.

    Plots the congestion grid as a colour heatmap with component
    bounding boxes overlaid. Saves to the given path if specified.

    Args:
        model: BoardModel with components and nets.
        grid_resolution: Cell size in mm (default 2.0).
        path: If specified, save the figure to this file path.

    Returns:
        The matplotlib figure object, or None if matplotlib is
        unavailable.
    """
    try:
        import matplotlib
    ...
    im = ax.imshow(
        grid,
        origin='lower',
        extent=[x_offset, x_offset + nx * cell_size,
                y_offset, y_offset + ny * cell_size],
        cmap='YlOrRd',
        aspect='equal',
    )
    fig.colorbar(im, ax=ax, label='RUDY congestion')

    # Overlay component bounding boxes
    for comp in model.components:
        if comp.is_fixed:
            colour = 'blue'
            alpha = 0.3
    ...
    ax.set_xlabel('X (mm)')
    ax.set_ylabel('Y (mm)')
    ax.set_title('RUDY Congestion Map')
